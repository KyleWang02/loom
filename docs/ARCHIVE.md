...
**Focus**: Phase 13 (Lint Engine) + Phase 14 (Doc Generation) + Real-World Demo Harness

**Completed**:
- **Phase 13 — Lint Engine**: Full implementation in `namespace loom::lint`:
  - `include/loom/lint.hpp` — Severity, LintDiagnostic, LintRule (abstract base with 4 check levels), SuppressionMap, LintReport, LintEngine
  - `src/util/lint.cpp` — 19 active rules + 3 deferred stubs (~520 lines)
  - 4 check levels: check(DesignUnit), check_file(ParseResult), check_tokens(LexResult), check_project(all_files)
  - SuppressionMap: same-line + next-line suppression from `// loom: ignore[rule-id]`, wildcard `*`
  - NamingRule base class with dynamic_cast for regex pattern configuration
  - Signal analysis rules skip modules with instantiations (can't see port connections)
  - 60 test cases, 213 assertions — all passing
- **Phase 14 — Doc Generation**: Full implementation in `namespace loom::doc`:
  - `include/loom/doc.hpp` — DocTag (8 kinds), DocComment, DocExtractor, PortDoc, ParamDoc, DesignUnitDoc, DocModel, MarkdownRenderer
  - `src/util/doc.cpp` — Doc comment parsing, extraction, cross-ref resolution, Markdown rendering
  - DocExtractor: find_leading_doc (consecutive /// blocks), find_trailing_doc (same-line ///)
  - Priority: @port/@param tag > trailing /// comment for port/param descriptions
  - DocModel: units + cross-ref resolution (instantiated_by populated from instantiates)
  - MarkdownRenderer: index.md, per-unit pages, Mermaid graphs, search_index.json
  - 50 test cases, 273 assertions — all passing
- **Real-World Demo Harness** (`demos/loom_demo.cpp`):
  - Single `loom-demo` executable with 3 subcommands: lint, filelist, resolve
  - Recursively scans directories for .v/.sv/.vl/.vlg files
  - Supports flags: --json, --rule, --config, --top, --save
  - Built and verified working on all subcommands
- **Sample Files** for testing the demo harness:
  - `demos/samples/lint/` — 10 Verilog files targeting specific lint rules
  - `demos/samples/filelist/` — 6-file module hierarchy
  - `demos/samples/resolve/` — Setup script creating 3 local git repos as dependencies
- All 30 test executables pass (1240+ total assertions), 0 regressions

**Key Decisions**:
- Inside `loom::lint` namespace, must use `loom::lex()` and `loom::parse()` (qualified) since bare names collide
- 3 rules deferred as no-op stubs: empty-port-connection, missing-port-connection, missing-begin-end (need richer parser data)
- Doc generation uses Markdown as primary output (HTML renderer deferred to Phase 16)
- Demo harness is a single executable (loom-demo) rather than 3 separate binaries

**Checkpoint Status**: Phases 0-14 complete. All tests pass.

**Files Changed**:
- `include/loom/lint.hpp` (new)
- `src/util/lint.cpp` (new)
- `tests/test_lint.cpp` (new)
- `include/loom/doc.hpp` (new)
- `src/util/doc.cpp` (new)
- `tests/test_doc.cpp` (new)
- `demos/loom_demo.cpp` (new)
- `demos/demo_lint.cpp` (new)
- `demos/demo_resolver.cpp` (new)
- `demos/demo_filelist.cpp` (new)
- `demos/samples/lint/*.sv, *.v` (new)
- `demos/samples/filelist/*.sv` (new)
- `demos/samples/resolve/setup.sh` (new)
